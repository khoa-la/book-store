...
public class SearchByCategoryController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int search= Integer.parseInt(request.getParameter("searchByCate"));
            DAO dao= new DAO();
            List<BookDTO> list= dao.getListBooksByCategory(search);
            if(!list.isEmpty()){
                request.setAttribute("LIST_BOOK_BY_CATE", list);
                url= SUCCESS;
            }
        } catch (Exception e) {
