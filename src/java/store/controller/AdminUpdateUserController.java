 ...
 * @author lakho
 */
@WebServlet(name = "AdminUpdateUserController", urlPatterns = {"/AdminUpdateUserController"})
public class AdminUpdateUserController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if ("admin".equals(userID) && statusID==0) {
                HttpSession session = request.getSession();
                session.setAttribute("ERROR_MESSAGE", "Admin status id can not be 0");
            } else {
                DAO dao = new DAO();
